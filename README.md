...
- 舵机 1：PA0 / TIM2_CH1，当前按 270 度舵机调试，0～270 度映射到 500～2500μs；上电默认 0 度，遥控 UP/DOWN 每次增减 45 度。舵机 2：PA1 / TIM2_CH2，暂时保持 0～180 度和 1000～2000μs。两路均为 50Hz，并使用独立 5V/6V 电源。
...
- `1~4` 分别独立启停 M1~M4；启动时 PWM 缓升，停止时立即归零。
- `5 (0x18)` 同时启停四路直流电机；存在单路未启动时按下会统一启动四路，四路均启动时按下会统一停止。
- `6 (0x5E)` 切换四轮统一正反方向；仅在四路电机全部停止时有效，禁止带载直接换向。
- 直流电机遥控调试占空比固定为 `75/99`，开机默认停留在“减速电机”调试页面。
- `RIGHT (0x43)` 设置 X 轴正转，`LEFT (0x44)` 设置 X 轴反转。
- 当前舵机调试阶段，`UP (0x46)` 将 PA0 舵机增加 45 度，`DOWN (0x15)` 减少 45 度；范围限制为 0～270 度，暂时覆盖原 Z 轴方向操作。
