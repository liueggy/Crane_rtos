...
- 遥控器 `UP (0x46)` 已绑定三步抓取演示：110 度张开、45 度抓取、65 度释放；每次短按推进一步，完成后循环。
- `DOWN (0x15)` 暂时用于每次减小 10 度，`POWER (0x45)` 暂时翻转 PE5 状态灯；其他按键等待后续分配。
...
- 当前 NEC 遥控按键：每次按 `UP (0x46)` 推进一次 PA0 夹爪演示，依次为 110 度完全张开、45 度闭合抓取、65 度释放豆子，之后循环；`DOWN (0x15)` 暂时保留每次减小 10 度的手动微调；`POWER (0x45)` 翻转状态灯 PE5。
