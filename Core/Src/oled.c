 ...
 * @note
 * 使用流程:
 * 1. STM32初始化IIC完成后调用OLED_Init(&hi2cX, 0x78) 初始化OLED. 注意STM32启动比OLED上电快, 可等待20ms再初始化OLED
...
// OLED参数
#if (OLED_HEIGHT % 8u) != 0u
#error "OLED_HEIGHT must be a multiple of 8"
#endif
#define OLED_PAGE (OLED_HEIGHT / 8u) // OLED页数
#define OLED_ROW OLED_HEIGHT         // OLED行数
#define OLED_COLUMN OLED_WIDTH       // OLED列数

// 显存
uint8_t OLED_GRAM[OLED_PAGE][OLED_COLUMN];

static I2C_HandleTypeDef *s_oled_i2c = NULL;
static uint16_t s_oled_address = 0;
static osMutexId_t s_oled_i2c_mutex = NULL;
static const osMutexAttr_t s_oled_i2c_mutex_attr = {
    .name = "oledI2cMutex",
};

#define OLED_I2C_TX_TIMEOUT_MS 100u

static uint8_t OLED_KernelCanBlock(void)
{
  osKernelState_t state = osKernelGetState();
  return (state == osKernelRunning);
}

static void OLED_EnsureMutex(void)
{
  if (s_oled_i2c_mutex == NULL && osKernelGetState() != osKernelInactive)
  {
    s_oled_i2c_mutex = osMutexNew(&s_oled_i2c_mutex_attr);
  }
}

static void OLED_Delay(uint32_t delay_ms)
{
  if (OLED_KernelCanBlock())
  {
    osDelay(delay_ms);
  }
  else
...
static void OLED_LockI2C(void)
{
  OLED_EnsureMutex();
  if (s_oled_i2c_mutex != NULL && OLED_KernelCanBlock())
  {
    (void)osMutexAcquire(s_oled_i2c_mutex, osWaitForever);
  }
}

static void OLED_UnlockI2C(void)
{
  if (s_oled_i2c_mutex != NULL && OLED_KernelCanBlock())
  {
    (void)osMutexRelease(s_oled_i2c_mutex);
  }
}
 ...
 */
static void OLED_Send(const uint8_t *data, uint16_t len)
{
  if (s_oled_i2c == NULL)
    return;
  OLED_LockI2C();
  (void)HAL_I2C_Master_Transmit(s_oled_i2c, s_oled_address, (uint8_t *)data, len, OLED_I2C_TX_TIMEOUT_MS);
  OLED_UnlockI2C();
}
 ...
 * @param hi2c 绑定的I2C句柄
 * @param address HAL使用的左移后7位地址, 例如SSD1306常用0x78
 */
void OLED_Init(I2C_HandleTypeDef *hi2c, uint16_t address)
{
  s_oled_i2c = hi2c;
...
void OLED_PlayAnimation(uint8_t x, uint8_t y, const Animation *anim, uint16_t count)
{
  ...
  uint32_t frameStart;
  uint32_t elapsed;
  while (count == 0 || played < count) {
    for (uint16_t i = 0; i < anim->frameCount; i++) {
      frameStart = HAL_GetTick();
      OLED_NewFrame();
      OLED_DrawImage(x, y, &anim->frames[i], OLED_COLOR_NORMAL);
      OLED_ShowFrame();
      elapsed = HAL_GetTick() - frameStart;
      if (elapsed < anim->frameDelay) {
        OLED_Delay(anim->frameDelay - elapsed);
      }
    }
    played++;
  }
